...
;(function($, window, document, undefined) {	
	...
	methods = {
           	...
           	return this.each(function() {
                ...
                if(typeof(opt) == 'undefined') {
                	var defaults = {
						...
						width: 300,             //弹框默认宽度
						height: 280,            //弹框默认高度
						positions: 'center',    //弹框位置(默认center：居中，top：顶部居中，left：顶部居;)
						triggerEvent: 'click',  //触发方式(默认click：点击，mouseenter：悬;)
						effect: 'hide',         //弹框关闭效果(默认hide，淡出关闭：fadeOut)
						resetForm: true,        //是否清空表单(默认true：清空，false：不清空)
						modalHide: true,        //是否点击遮罩背景关闭弹框(默认true：关闭，false：不可关;)						
                		closeHide: true,        //是否隐藏关闭按钮(默认true：不隐藏，false：隐;)
                		escHide: true,          //是否支持ESC关闭弹框(默认true：关闭，false：不可关;)
        ...
        add: function (o,$this) { //显示弹框
			...
			if(o.title != ''){ $obj.stop().prepend('<div id="HTitle" style="padding:10px 45px 10px 12px;border-bottom:1px solid #ddd;background-color:rgb(14,124,161);color:white;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+o.title+'</div>'); }
